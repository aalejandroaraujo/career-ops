...
2. **For each surviving pending URL**:
   ...
   **Tuning it:** Generating a tailored PDF costs ~30–60s per entry (AltaCV payload build + pdfLaTeX compile — see `modes/pdf.md`, which writes `output/cv-{candidate}-{company-slug}-{YYYY-MM-DD}.pdf`) and produces files that often go unused — most roles score in the 2.x/3.x range and never reach the application stage. Raise `auto_pdf_score_threshold` (e.g. `4.0`) to write only the report for marginal offers and produce the PDF on demand via `/career-ops pdf {slug}`; set `0` to generate one for every offer. Both modes (Path A `/career-ops pipeline` and Path B `batch/batch-runner.sh`) read the same key, so behavior is identical regardless of which path processes an offer.
